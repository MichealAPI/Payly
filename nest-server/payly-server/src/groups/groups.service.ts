...
export class GroupsService {
    ...
    async upsertGroup(currentUser: User, upsertGroupDto: UpsertGroupDto): Promise<Group> {
        const filter = {
        ...
        return this.groupModel.findOneAndUpdate(filter, update, {
            upsert: true,
            new: true,
    ...
    async deleteGroup(currentUser: User, groupId: string): Promise<{ message: string }> {
    ...
    async retrieveGroup(currentUser: User, groupId: string): Promise<Group> {
        const group = await this.groupModel.findById(groupId).populate([
            {
                path: 'members',
                select: 'firstName lastName email _id'
            },
        ...
        if (!group.members.includes(currentUser._id as any) && !group.owner.equals(currentUser._id as any)) {
    ...
    async kickUser(currentUser: User, groupId: string, userId: string): Promise<{ message: string }> {
        ...
        if (group.expenses.some(expense => expense.paidBy.equals(userIdObject) || expense.splitDetails.some(detail => detail.user.equals(userIdObject)))) {
            throw new BadRequestException('Cannot kick user who is involved in expenses. Please resolve expenses first.');
        }

        group.members = group.members.filter(member => !member.equals(userIdObject));
        await group.save();
    ...
    async addExpense(groupId: string, expenseId: mongoose.Types.ObjectId): Promise<Group> {
        const group = await this.groupModel.findById(groupId).exec();
        ...
        if (group.expenses.includes(expenseId)) {
            throw new BadRequestException('Expense is already associated with this group');
        }
    ...
    async removeExpense(groupId: string, expenseId: mongoose.Types.ObjectId): Promise<Group> {
        const group = await this.groupModel.findById(groupId).exec();
        if (!group) {
            throw new NotFoundException('Group not found');
        }
        ...
        if (!group.expenses.includes(expenseId)) {
            throw new BadRequestException('Expense is not associated with this group');
        }

        group.expenses = group.expenses.filter(expense => !expense.equals(expenseId));
        await group.save();
        return group;
    }
    ...
    async isGroupMember(currentUser: User, groupId: string): Promise<boolean> {
        ...
        return group.members.includes(currentUser._id as any) || group.owner.equals(currentUser._id as any);
    ...
    async getUserGroupBalances(currentUser: User, groupId: string): Promise<any> {
        ...
        return calculateBalances(group.expenses, (currentUser._id as any).toString());
    }
}