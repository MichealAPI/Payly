...
const CurrencySelector = ({setCurrency, currency}) =>  {
    ...
    return (
        <>
            <Combobox value={selected} onChange={(currency) => setSelectedId(currency.id)} onClose={() => setQuery('')}>
                <div className="relative">
                    <ComboboxInput
                        ...
                        onChange={(event) => setQuery(event.target.value) && setCurrency(event.target.value)}
                    />
                    ...
                    {filteredCurrencies.map((currency) => (
                        ...
                        >
                            ...
                            <div className="text-sm/6 text-white font-bold flex w-full items-center">
                                <img src={`https://flagpedia.net/data/${currency.section}/w20/${currency.countryCode}.webp`} className='h-3 w-5 mr-2' alt={currency.name} />
                                {currency.name}
                            </div>
                        </ComboboxOption>
                    ))}
